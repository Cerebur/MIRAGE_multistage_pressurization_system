#!/usr/bin/env python3
"""
Serial logger that connects to the device, responds to the test_meta prompt,
...
Usage:
  .venv/bin/python tools/serial_logger.py --port auto --baud 115200 --meta yes

Options:
  --port   Serial port or 'auto' (default: auto)
  --baud   Baud rate (default: 115200)
  --meta   meta value to send when device prompts (test description)
...
def parse_args():
    ...
    p.add_argument('--meta', default='unknown', help='Value to send when prompted for test_meta')
    ...
    with open(outfile, 'w', newline='') as f:
        ...
        try:
            while True:
                ...
                if ('TEST META DATA' in line) or line.lower().startswith('test meta data'):
                    val = args.meta[0].lower()
                    if val == 'y':
                        tosend = 'y\n'
                    elif val == 'n':
                        tosend = 'n\n'
                        ...
                        tosend = (args.meta + '\n')
                    print('Sending test meta data:', tosend.strip())
